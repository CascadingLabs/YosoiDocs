...
Contract executors currently share the batching behavior of [`ys.js`](/guides/js-fields/): a per-field JavaScript exception is isolated as `null`, then normal field validation decides whether the record is valid. Executors inside a Flow use replay `EVAL` actions and propagate JavaScript failures directly.
...
- supports named function exports and static relative named imports and re-exports;
...
- An ordinary annotation on `ys.Executor.js` validates captured output.
- The executor attribute name becomes its `output_field`.
