...
For new hand-authored code, `ys.Executor.js(...)` provides a typed namespace, confined local `.js` and `.mjs` module loading, settle conditions, and integration with handwritten A3Node Flows. See [JavaScript Executors and Browser Flows](/guides/executor-js-flow/). `ys.js(...)` remains available and compatible.

There are two `ys.js` modes:
...
policy = ys.Policy.cascade(
    ys.Policy.from_env(),
    ys.Policy(scrape=ys.ScrapePolicy(fetcher_type="headless")),
)
